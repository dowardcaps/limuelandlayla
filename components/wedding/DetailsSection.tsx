...
function VenueBlock({
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 px-2 text-center sm:items-start sm:px-4 sm:text-left">
      <p className="tracked-wide text-mdtext-navy">{label}</p>
      <p className="font-display text-2xl italic text-navy sm:text-2xl">{venue}</p>
      <p className="text-lg text-navy">{time}</p>
      <p className="text-lg text-navy">{address}</p>
    </div>
  );
}
...
export default function DetailsSection() {
  return (
    <section className="mx-auto w-full max-w-3xl px-4 py-12 sm:px-6 sm:py-20 md:px-10">
      ...
      <div className="flex flex-col gap-8 border-t border-hairline pt-8 sm:gap-10 sm:pt-10">
